...
** default definition.
*/
// LUAI_FUNC是所有不被导出到外部模块的外部函数的标记
// LUAI_DDEF和LUAI_DDEC是所有不被导出到外部模块的外部常量的标记
// 如果你需要在一些特殊环境下就改变它们。例如：Elf/gcc(3.2及以上版本)在lua被编译为share库时，就把它们标记为hidden来优化访问
// 并不是所有的ELF格式都支持这个特性。不幸的是gcc不提供一个检查目标格式是否支持这个特性的方法，但是那些不支持的目标则给了一个警告。
// 为了避免这些警告把它们变为默认的定义

// 上面的意思就是说：在GCC的302版本及以后，LUAI_FUNC都是对外部模块不可见的标志，302以前则为extern
// 详情就去查看__attribute__的用法
